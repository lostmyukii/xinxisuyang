...
> 状态：手动导入正式版实施与本机验收已完成；自动采集真实验证仍未完成并保持关闭
>
> 日期：2026-07-20
>
...
| 阶段 | 状态 | 说明 |
|---|---|---|
| 阶段 0 工程基线 | 已完成 | pnpm workspace、统一命令、合成夹具和 Git 忽略均已落地 |
| 阶段 1 采集探针 | 已完成探针；真实验证未完成 | 扩展只能报告 `STRUCTURE_ONLY`，因此生产自动采集保持关闭 |
| 阶段 2–8 | 已完成 | 排名、手动导入、快照、后台、大屏和三类导出已通过自动化验收 |
| 阶段 9 | 已完成（手动分支） | 安全、安装、完整质量门及 API/UI 双通道 240 分钟稳定性验收均已通过 |
...
1. 自动采集只有取得真实 `GO` 后才实现并开放；届时自动同步和立即同步必须共用单队列。
2. 当前手动正式版的发布、规则重算和快照恢复使用串行发布边界，同一时刻最多一个候选发布任务。
3. 每次手动发布记录持久化 `SyncRun`，启动时恢复最近成功时间和中断任务状态。
4. 默认 120 秒无成功发布进入橙色过期状态，5 分钟进入红色严重警告，阈值可在本机调整。
...
1. 固化 Frost、Ink、System Blue、Action Blue、Green、Orange、Red 七个令牌。
...
2. 手动正式版只显示手动发布状态和最近成功时间；“立即同步”仅在自动采集真实 `GO` 后显示。
3. 新鲜度光环同时显示颜色、图标和文字。
4. 最新排名显示当前筛选分区和并列状态。
5. 异常面板直接进入对应异常列表。
...
7. 工作簿在内存中完整生成并通过结构校验后才作为响应发送；生成失败不产生看似完整的下载文件。
...
- 测试脚本在 macOS 使用 `caffeinate` 保持唤醒，并检测采样空档；内存以预热后首尾稳定窗口及滚动窗口判断持续增长。
...
7. 打开 16:9 大屏。
8. 生成三种 Excel 导出。
9. 手动正式版模拟字段变化、候选哈希变化和 SQLite 写入失败；只有自动采集取得 `GO` 后才演练页面关闭与登录失效。
10. 确认上一成功排名始终保留。
...
- 无效记录不参与排名且有具体原因。
- 自动采集为 `GO` 时，自动与立即同步共用完整校验和原子发布流程；采用手动正式版时，两者均不得显示或调用。
- 失败不会覆盖上一成功快照。
...
1. [x] 从零执行 API 与生产 UI 两条 240 分钟稳定性测试。
2. [x] 把开始时间、结束时间、样本数、瞬时失败和内存指标写入验收报告。
3. [x] 重新执行 `pnpm package:local`、依赖审计、敏感信息扫描和本机启动/退出检查。
4. [x] 提交并推送最终检查点，确认本地 `main` 与 `origin/main` 一致。
5. [ ] 现场由管理员配置真实赛项范围、核对金山总记录数并完成一次手动导入演练。

真实 Go/No-Go 条件未全部通过前，不宣传、包装或交付自动同步能力。
