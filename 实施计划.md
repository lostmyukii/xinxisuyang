...
> 状态：实施中；手动正式版功能与自动化质量门已完成，正式 4 小时稳定性验收待执行
>
> 日期：2026-07-19
>
...
| 阶段 9 | 进行中 | 安全、安装和短时稳定性烟测已通过；正式 API/UI 240 分钟测试待执行 |
...
1. 从零执行 API 与生产 UI 两条 240 分钟稳定性测试。
2. 把开始时间、结束时间、样本数、瞬时失败和内存增幅写入验收报告。
3. 重新执行 `pnpm package:local`、依赖审计、敏感信息扫描和本机启动/退出检查。
